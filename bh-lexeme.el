;;; bh-lexeme.el --- bh lexical tokens   -*- coding: utf-8; lexical-binding: t -*-
...
(unless (category-docstring ?P)
  (define-category ?P "Bh symbol constituent characters")
  (map-char-table
   #'(lambda (key val)
       (if (or
...
(defconst bh-lexeme-modid
  "[[:upper:]][[:alnum:]'_]*"
  "Regexp matching a valid Bh module identifier.
...
(defconst bh-lexeme-id
  "[[:alpha:]_][[:alnum:]'_]*"
  "Regexp matching a valid Bh identifier.
...
(defconst bh-lexeme-sym
  "\\cP+"
  "Regexp matching a valid Bh variable or constructor symbol.

GHC accepts a string of chars from the set
[:!#$%&*+./<=>?@^|~\\-] or Unicode category Symbol for chars with
codes larger than 128 only.")

(defconst bh-lexeme-idsym-first-char
  "\\(?:[[:alpha:]_]\\|\\cP\\)"
...
(defconst bh-lexeme-modid-opt-prefix
  (concat "\\(?:" bh-lexeme-modid "\\.\\)*")
  "Regexp matching a valid Bh module prefix, potentially empty.

Module path prefix is separated by dots and finishes with a
dot. For path component syntax see `bh-lexeme-modid'.")

(defconst bh-lexeme-qid-or-qsym
  (rx-to-string `(: (regexp ,bh-lexeme-modid-opt-prefix)
                    (group (| (regexp ,bh-lexeme-id) (regexp ,bh-lexeme-sym)
                              ))))
  "Regexp matching a valid qualified identifier or symbol.

Note that (match-string 1) returns the unqualified part.")

(defun bh-lexeme-looking-at-qidsym ()
...
the unqualified part (if any)."
  (let ((begin (point))
    ...
    (save-excursion
      (while (looking-at (concat bh-lexeme-modid "\\."))
        (goto-char (match-end 0)))
      (cond
       ((looking-at bh-lexeme-id)
...
(defun bh-lexeme-looking-at-backtick ()
  ...
  (let ((match-data-old (match-data t))
    ...
    (save-excursion
      (when (looking-at "`")
        ...
        (when (bh-lexeme-looking-at-qidsym)
...
(defconst bh-lexeme-qid
  (rx-to-string `(: (regexp "'*")
                    (regexp ,bh-lexeme-modid-opt-prefix)
                    (group (regexp ,bh-lexeme-id))))
  "Regexp matching a valid qualified identifier.

Note that (match-string 1) returns the unqualified part.")

(defconst bh-lexeme-qsym
  (rx-to-string `(: (regexp "'*")
                    (regexp ,bh-lexeme-modid-opt-prefix)
                    (group (regexp ,bh-lexeme-id))))
...
(defconst bh-lexeme-char-literal-inside
  (rx (| (not (any "\n'\\"))
         (: "\\"
            (| "a" "b" "f" "n" "r" "t" "v" "\\" "\"" "'"
...
Note that `bh-lexeme-char-literal-inside' matches strictly
only escape sequences defined in Bh Report.")

(defconst bh-lexeme--char-literal-rx
  (rx-to-string `(: (group "'")
...
Note that `bh-lexeme--char-literal-rx' matches more than
Bh Report specifies because we want to support also code
under edit.
...
(defun bh-lexeme-looking-at-char-literal ()
  "Non-nil when point is at a char literal lookalike.

Note that this function matches more than Bh Report
...
After successful match:
 ...
 (match-text 3) matches the closing quote, or a closing
  ...
  (when (looking-at bh-lexeme--char-literal-rx)
    (set-match-data
...
(defconst bh-lexeme-string-literal-inside-item
  (rx (| (not (any "\n\"\\"))
         (: "\\"
            (| "a" "b" "f" "n" "r" "t" "v" "\\" "\"" "'" "&"
               "NUL" "SOH" "STX" "ETX" "EOT" "ENQ" "ACK"
...
Note that `bh-lexeme-string-literal-inside-item' matches
strictly only escape sequences defined in Bh Report.")

(defconst bh-lexeme-string-literal
  (rx (: (group "\"")
         (group (* (| (regexp "\\\\[ \t\n\r\v\f]*\\\\")
                      (regexp "\\\\[ \t\n\r\v\f]+")
                      (regexp "\\\\[^ \t\n\r\v\f]")
...
Note that `bh-lexeme-string-literal' matches more than
Bh Report specifies because we want to support also code
under edit.
...
(defun bh-lexeme-looking-at-string-literal ()
...
(defun bh-lexeme-looking-at-quasi-quote-literal ()
  "Non-nil when point is just in front of Template Bh
quaisquote literal.
...
as quoter names according to Template Bh specification."
  (let ((match-data-old (match-data t)))
    (if (and
         (looking-at (rx-to-string `(: "[" (optional "$")
                                       ...
                                       (group (regexp ,bh-lexeme-id))
                                       (group "|"))))
         (equal (bh-lexeme-classify-by-first-char (char-after (match-beginning 1)))
                'varid)
...
(defun bh-lexeme-classify-by-first-char (char)
...
(defun bh-lexeme-looking-at-token (&rest flags)
  "Like `looking-at' but understands Bh lexemes.

Moves point forward over whitespace.  Returns a symbol describing
type of Bh token recognized.  Use `match-string',
...
- \\='template-bh-quote: for a string of apostrophes for template
   bh
- \\='template-bh-quasi-quote: for a string of apostrophes for template
   bh
...
See `bh-lexeme-classify-by-first-char' for details."
  (while
    ...
    (or
     ...
     (and (looking-at "{-")
          (save-excursion
            ...
            'nested-comment))
     (and (bh-lexeme-looking-at-char-literal)
          'char)
     (and (bh-lexeme-looking-at-string-literal)
          'string)
     (and (looking-at "[][(){}`,;]")
          (if (bh-lexeme-looking-at-quasi-quote-literal)
              'template-bh-quasi-quote
            'special))
     (and (bh-lexeme-looking-at-qidsym)
          (if (save-match-data
              ...
              (progn
                ...
                'comment)
            'qsymid))
     (and (looking-at bh-lexeme-number)
          'number)
     (and (looking-at "'+")
          'template-bh-quote)
     (and (looking-at ".")
