;;; bh-font-lock.el --- Font locking module for Bh Mode -*- lexical-binding: t -*-
...
(defgroup bh-appearance nil
  "Bh Appearance."
  :group 'bh)


(defcustom bh-font-lock-symbols nil
...
(defcustom bh-font-lock-symbols-alist
  '(("\\" . "λ")
    ("not" . "¬")
    ("->" . "→")
    ("<-" . "←")
    ...
    ("." "∘" ; "○"
     ...
     ;; forall <foo> . <bar>.
     bh-font-lock-dot-is-not-composition)
    ("forall" . "∀"))
  "Alist mapping Bh symbols to chars.

Each element has the form (STRING . COMPONENTS) or (STRING
COMPONENTS PREDICATE).

STRING is the Bh symbol.
...
(defcustom bh-font-lock-keywords
  ...
  '("module" "rules" "package" "when"
    ...
    "anyclass" "stock" "via")
  "Identifiers treated as reserved keywords in Bh."
  :group 'bh-appearance
  :type '(repeat string))


(defun bh-font-lock-dot-is-not-composition (start)
...
(defvar bh-yesod-parse-routes-mode-keywords
  '(("^\\([^ \t\n]+\\)\\(?:[ \t]+\\([^ \t\n]+\\)\\)?"
     (1 'font-lock-string-face)
     (2 'bh-constructor-face nil lax))))

(define-derived-mode bh-yesod-parse-routes-mode text-mode "Yesod parseRoutes mode"
  "Mode for parseRoutes from Yesod."
  (setq-local font-lock-defaults '(bh-yesod-parse-routes-mode-keywords t t nil nil)))

(defcustom bh-font-lock-quasi-quote-modes
  `(("hsx" . xml-mode)
    ...
    ("json" . json-mode)
    ("aesonQQ" . json-mode)
    ("parseRoutes" . bh-yesod-parse-routes-mode))
  "Mapping from quasi quoter token to fontification mode.

If a quasi quote is seen in Bh code its contents will have
font faces assigned as if respective mode was enabled."
  :group 'bh-appearance
  :type '(repeat (cons string symbol)))

(defface bh-keyword-face
  '((t :inherit font-lock-keyword-face))
  "Face used to highlight Bh keywords."
  :group 'bh-appearance)

(defface bh-type-face
  '((t :inherit font-lock-type-face))
  "Face used to highlight Bh types"
  :group 'bh-appearance)

(defface bh-constructor-face
  '((t :inherit font-lock-type-face))
  "Face used to highlight Bh constructors."
  :group 'bh-appearance)
...
(defface bh-definition-face
  '((t :inherit font-lock-function-name-face))
  "Face used to highlight Bh definitions."
  :group 'bh-appearance)

;; This is probably just wrong, but it used to use
;; `font-lock-function-name-face' with a result that was not consistent with
;; other major modes, so I just exchanged with `bh-definition-face'.
(defface bh-operator-face
  '((t :inherit font-lock-variable-name-face))
  "Face used to highlight Bh operators."
  :group 'bh-appearance)

(defface bh-pragma-face
  '((t :inherit font-lock-preprocessor-face))
  "Face used to highlight Bh pragmas ({-# ... #-})."
  :group 'bh-appearance)

(defface bh-liquid-bh-annotation-face
  '((t :inherit bh-pragma-face))
  "Face used to highlight LiquidBh annotations ({-@ ... @-})."
  :group 'bh-appearance)

(defface bh-literate-comment-face
  '((t :inherit font-lock-doc-face))
  "Face with which to fontify literate comments.
Inherit from `default' to avoid fontification of them."
  :group 'bh-appearance)

(defface bh-quasi-quote-face
  '((t :inherit font-lock-string-face))
  "Generic face for quasiquotes.

Some quote types are fontified according to other mode defined in
`bh-font-lock-quasi-quote-modes'."
  :group 'bh-appearance)

(defun bh-font-lock-compose-symbol (alist)
...
(defun bh-font-lock-symbols-keywords ()
  (when (and bh-font-lock-symbols
             bh-font-lock-symbols-alist)
    `((,(regexp-opt (mapcar 'car bh-font-lock-symbols-alist) t)
       (0 (bh-font-lock-compose-symbol ',bh-font-lock-symbols-alist)
          ;; In Emacs-21, if the `override' field is nil, the face
          ;; expressions is only evaluated if the text has currently
          ;; no face.  So force evaluation by using `keep'.
          keep)))))

(defun bh-font-lock--forward-type (&optional ignore)
...
like ::, class, instance, data, newtype, type."
  ...
  (let ((cont t)
        ...
        (open-parens 0))
    (while cont
      (setq token (bh-lexeme-looking-at-token 'newline))
      ...
      (cond
       ...
       ((and (or (member (match-string-no-properties 0)
                         '("<-" "=" "←"))
                 (member (match-string-no-properties 0) bh-font-lock-keywords))
             (not (member (match-string-no-properties 0) ignore)))
        (setq cont nil)
        (setq last-token-was-newline nil))
       ((member (match-string-no-properties 0)
        ...
        (if last-token-was-newline
          ...
          (goto-char end)
          (setq last-token-was-newline nil)))
       ((member token '(qsymid char string number template-bh-quote template-bh-quasi-quote))
        (setq last-token-was-operator (member (bh-lexeme-classify-by-first-char (char-after (match-beginning 1)))
                                              '(varsym consym)))
...
(defun bh-font-lock--select-face-on-type-or-constructor ()
  "Private function used to select either type or constructor face
on an uppercase identifier."
  (cl-case (bh-lexeme-classify-by-first-char (char-after (match-beginning 1)))
                     ...
                     (bh-font-lock--forward-type
                      (cond
                       ...
                       ((member word '("type"))
                        ...
                        '("=" "instance")))))
                   (add-text-properties (match-end 0) (point) '(font-lock-multiline t bh-type t))))
               'bh-keyword-face)
              ((member word '("forall"))
               (when (get-text-property (match-beginning 0) 'bh-type)
                 'bh-keyword-face)))))
    (conid (if (get-text-property (match-beginning 0) 'bh-type)
               'bh-type-face
             'bh-constructor-face))
    ...
    (consym (if (not (member (match-string 1) '("::" "∷")))
                (if (get-text-property (match-beginning 0) 'bh-type)
                    'bh-type-face
                  'bh-constructor-face)
              (save-excursion
                (goto-char (match-end 0))
...
(defun bh-font-lock--put-face-on-type-or-constructor ()
  "Private function used to put either type or constructor face
on an uppercase identifier."
  (let ((face (bh-font-lock--select-face-on-type-or-constructor)))
    (when (and face
  ...
  (let* (;; Bird-style literate scripts start a line of code with
    ...
    (setq keywords
          `(;; NOTICE the ordering below is significant
            ...
            ,@(bh-font-lock-symbols-keywords)

            ;; Special case for `as', `hiding', `safe' and `qualified', which are
            ;; keywords in import statements but are not otherwise reserved.
            ("\\<import[ \t]+\\(?:\\(safe\\>\\)[ \t]*\\)?\\(?:\\(qualified\\>\\)[ \t]*\\)?\\(?:\"[^\"]*\"[\t ]*\\)?[^ \t\n()]+[ \t]*\\(?:\\(qualified\\>\\)[ \t]*\\)?\\(?:\\(\\<as\\>\\)[ \t]*[^ \t\n()]+[ \t]*\\)?\\(\\<hiding\\>\\)?"
             (1 'bh-keyword-face nil lax)
             (2 'bh-keyword-face nil lax)
             (3 'bh-keyword-face nil lax)
             (4 'bh-keyword-face nil lax)
             (5 'bh-keyword-face nil lax))
            ...
            (,topdecl-var  (1 (unless (member (match-string 1) bh-font-lock-keywords)
                                'bh-definition-face)))
            (,topdecl-var2 (2 (unless (member (match-string 2) bh-font-lock-keywords)
                                'bh-definition-face)))
            (,topdecl-bangpat  (1 (unless (member (match-string 1) bh-font-lock-keywords)
                                'bh-definition-face)))
            ...
            ("`"
             (0 (if (or (elt (syntax-ppss) 3) (elt (syntax-ppss) 4))
                  ...
                  (when (save-excursion
                          ...
                          (bh-lexeme-looking-at-backtick))
                    (goto-char (match-end 0))
                    (unless (text-property-not-all (match-beginning 1) (match-end 1) 'face nil)
                      (put-text-property (match-beginning 1) (match-end 1) 'face 'bh-operator-face))
                    (unless (text-property-not-all (match-beginning 2) (match-end 2) 'face nil)
                      (put-text-property (match-beginning 2) (match-end 2) 'face 'bh-operator-face))
                    (unless (text-property-not-all (match-beginning 4) (match-end 4) 'face nil)
                      (put-text-property (match-beginning 4) (match-end 4) 'face 'bh-operator-face))
                    (add-text-properties
                     (match-beginning 0) (match-end 0)
                     '(font-lock-fontified t fontified t font-lock-multiline t))))))

            (,bh-lexeme-idsym-first-char
             (0 (if (or (elt (syntax-ppss) 3) (elt (syntax-ppss) 4))
                    (parse-partial-sexp (point) (point-max) nil nil (syntax-ppss)
                                        'syntax-table)
                  (when (save-excursion
                          (goto-char (match-beginning 0))
                          (bh-lexeme-looking-at-qidsym))
                    (goto-char (match-end 0))
                    ;; note that we have to put face ourselves here because font-lock
                    ;; will use match data from the original matcher
                    (bh-font-lock--put-face-on-type-or-constructor)))))))
    keywords))


(defun bh-font-lock-fontify-block (lang-mode start end)
  ...
  (let ((string (buffer-substring-no-properties start end))
    ...
    (with-current-buffer
        (get-buffer-create
         (concat " bh-font-lock-fontify-block:" (symbol-name lang-mode)))
...
(defun bh-syntactic-face-function (state)
  "`font-lock-syntactic-face-function' for Bh."
  (cond
   ((nth 3 state)
    (if (equal ?| (nth 3 state))
        ...
        (let* ((qqname (save-excursion
               ...
               (lang-mode (cdr (assoc (bh-string-drop-qualifier qqname)
                                      bh-font-lock-quasi-quote-modes))))
          ...
          (if (and lang-mode
              ...
              (save-excursion
                ...
                (parse-partial-sexp (point) (point-max) nil nil state
                                    'syntax-table)
                (bh-font-lock-fontify-block lang-mode (1+ (nth 8 state)) (1- (point)))
      ...
      (save-excursion
        (let
          ...
          (while (re-search-forward "\\\\" end-of-string t)
            ...
            (if (looking-at bh-lexeme-string-literal-inside-item)
                (goto-char (match-end 0))
   ...
   ((save-excursion
      ...
      (and (looking-at-p "{-#")
           ...
           (looking-at-p "#-}")))
    'bh-pragma-face)
   ;; Detect Liquid Bh annotations enclosed in special comment
   ;; delimiters {-@ .. @-}.
   ((save-excursion
      (goto-char (nth 8 state))
      (and (looking-at-p "{-@")
...
(defun bh-font-lock-defaults-create ()
  "Locally set `font-lock-defaults' for Bh."
...
(defun bh-fontify-as-mode (text mode)
