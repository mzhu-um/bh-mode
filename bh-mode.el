...
(require 'bh-string)
(require 'bh-indentation)
(require 'bh-font-lock)
;; (require 'haskell-cabal)

;; All functions/variables start with `(literate-)haskell-'.

;; Version of mode.
(defconst bh-version "16.2-git"
  "The release version of `bh-mode'.")

;;;###autoload
(defun bh-version (&optional here)
  "Show the `bh-mode` version in the echo area.
With prefix argument HERE, insert it at point."
  (interactive "P")
  (let* ((bh-mode-dir (ignore-errors
                             (file-name-directory (or (locate-library "bh-mode") ""))))
         (version (format "bh-mode version %s (%s)"
                           bh-version
                           bh-mode-dir)))
    (if here
        (insert version)
...
;;;###autoload
(defun bh-mode-view-news ()
  "Display information on recent changes to bh-mode."
  (interactive)
  (with-current-buffer (find-file-read-only (expand-file-name "NEWS" bh-mode-pkg-base-dir))
    (goto-char (point-min))
...
;; (defvar-local bh-literate nil
;;   "If not nil, the current buffer contains a literate Bh script.
;; Possible values are: `bird' and `tex', for Bird-style and LaTeX-style
;; literate scripts respectively.  Set by `bh-mode' and
;; `bh-literate-mode'.  For an ambiguous literate buffer -- i.e. does
;; not contain either \"\\begin{code}\" or \"\\end{code}\" on a line on
;; its own, nor does it contain \">\" at the start of a line -- the value
;; of `bh-literate-default' is used.")
;; (put 'bh-literate 'safe-local-variable 'symbolp)

;; Default literate style for ambiguous literate buffers.
;; (defcustom bh-literate-default 'bird
;;   "Default value for `bh-literate'.
...
(defvar bh-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    ;; (define-key map (kbd "C-c C-,") 'bh-mode-format-imports)
    ;; (define-key map [remap delete-indentation] 'bh-delete-indentation)
    ;; (define-key map (kbd "C-c C-l") 'bh-mode-enable-process-minor-mode)
    ;; (define-key map (kbd "C-c C-b") 'bh-mode-enable-process-minor-mode)
    ;; (define-key map (kbd "C-c C-v") 'bh-mode-enable-process-minor-mode)
    ;; (define-key map (kbd "C-c C-t") 'bh-mode-enable-process-minor-mode)
    ;; (define-key map (kbd "C-c C-i") 'bh-mode-enable-process-minor-mode)
    ;; (define-key map (kbd "C-c C-s") 'bh-mode-toggle-scc-at-point)
    map)
  "Keymap used in `bh-mode'.")

;; (defun bh-mode-enable-process-minor-mode ()
;;   "Tell the user to choose a minor mode for process interaction."
;;   (interactive)
;;   (error "Run `C-h f bh-mode` for instruction how to setup a Bh interaction mode."))

;; (easy-menu-define bh-mode-menu bh-mode-map
;;   "Menu for the Bh major mode."
...
;;     "---"
;;     ["Start interpreter" bh-interactive-switch]
;;     ["Load file" bh-process-load-file]
;;     "---"
;;     ["Load tidy core" ghc-core-create-core]
;;     "---"
;;     ["Doc mode" bh-doc-mode
;;      :style toggle :selected (and (boundp 'bh-doc-mode) bh-doc-mode)]
;;     ["Customize" (customize-group 'bh)]
;;     ))
...
(defvar bh--char-syntax-identifiers
  '(170
...
(defvar bh-mode-syntax-table
  (let ((table (make-syntax-table)))
    ...
    ;; Bh symbol characters are treated as punctuation because
    ;; they are not able to form identifiers with word constituent 'w'
    ;; class characters.
    (dolist (charcodes bh--char-syntax-symbols)
      (modify-syntax-entry charcodes "." table))
    ;; ... and for identifier characters
    (dolist (charcodes bh--char-syntax-identifiers)
      (modify-syntax-entry charcodes "w" table))

    table)
  "Syntax table used in Bh mode.")

(defun bh-syntax-propertize (begin end)
  (save-excursion
    (when bh-literate
      (goto-char begin)
    ...
    (let ((ppss (syntax-ppss)))
      ...
      (while (< (point) end)
        (let
            ((token-kind (bh-lexeme-looking-at-token)))

          (cond
           ((equal token-kind 'qsymid)
            (when (member
                   (bh-lexeme-classify-by-first-char (char-after (match-beginning 1)))
                   '(varsym consym))
           ...
           ((equal token-kind 'string)
            (save-excursion
              ...
              (when (and (not (match-beginning 3))
                ...
                (put-text-property (match-beginning 1) (match-end 1) 'syntax-table (string-to-syntax "|"))
                (put-text-property (match-end 2 ) (1+ (match-end 2)) 'syntax-table (string-to-syntax "|")))))
           ((equal token-kind 'template-bh-quasi-quote)
...
(defun bh-ident-at-point ()
  "Return the identifier near point going backward or nil if none found.
May return a qualified name."
  (let ((reg (bh-ident-pos-at-point)))
    (when reg
      (buffer-substring-no-properties (car reg) (cdr reg)))))

(defun bh-spanable-pos-at-point ()
  "Like `bh-ident-pos-at-point', but includes any surrounding backticks."
  (save-excursion
    (let ((pos (bh-ident-pos-at-point)))
      (when pos
  ...
  (when (not (nth 8 (syntax-ppss)))
    ...
    (let (start end)
      ...
      (save-excursion
        ...
        (save-excursion
          (if (not (bh-mode--looking-at-varsym))
          ...
          (when (looking-at-p (rx "."))
            (forward-char))
          (let ((pos (bh-mode--skip-qualification-backward)))
            (when pos
              (setq start pos))))
        ...
        (save-excursion
          ...
          ;; extra check.
          (while (bh-mode--looking-at-varsym)
            (forward-char)
            (setq end (point))))
        (when (not (= start end))
          (cons start end))))))

(defun bh-mode--looking-at-varsym ()
  "Return t when point stands at operator symbol."
  (when (not (eobp))
    (let ((lex (bh-lexeme-classify-by-first-char (char-after))))
      (or (eq lex 'varsym)
          (eq lex 'consym)))))

(defun bh-mode--skip-qualification-backward ()
...
Note that  `bh-indentation-mode' and `bh-indent-mode' should not be
...
;;;###autoload
(define-derived-mode bh-mode prog-mode "Bh"
  "Major mode for editing Bh programs.

\\<bh-mode-map>

Literate Bh scripts are supported via `bh-literate-mode'.
The variable `bh-literate' indicates the style of the script in the
current buffer.  See the documentation on this variable for more details.

Use `bh-version' to find out what version of Bh mode you are
currently using.

Additional Bh mode modules can be hooked in via `bh-mode-hook'.

Indentation modes:

    `bh-indentation-mode', Kristof Bastiaensen, Gergely Risko
...
Other modes:
    ...
    `bh-decl-scan-mode', Graeme E Moss
      Scans top-level declarations, and places them in a menu.

    `bh-doc-mode', Hans-Wolfgang Loidl
...
- `smerge-mode': show and work with diff3 conflict markers used
  ...
  (when (version< emacs-version "25.1")
    (error "bh-mode requires at least Emacs 25.1"))
  ...
  (setq-local fill-paragraph-function 'bh-fill-paragraph)
  ;; (setq-local adaptive-fill-function 'bh-adaptive-fill)
  (setq-local comment-start "--")
  (setq-local comment-padding 1)
  (setq-local comment-start-skip "[-{]-[ \t]*")
  (setq-local comment-end "")
  (setq-local comment-end-skip "[ \t]*\\(-}\\|\\s>\\)")
  (setq-local forward-sexp-function #'bh-forward-sexp)
  (setq-local parse-sexp-ignore-comments nil)
  (setq-local syntax-propertize-function #'bh-syntax-propertize)

  ;; Set things up for imenu.
  (setq-local imenu-create-index-function 'bh-ds-create-imenu-index)
  ;; Set things up for font-lock.
  (setq-local font-lock-defaults
              '((bh-font-lock-keywords)
                nil nil nil nil
                (font-lock-syntactic-face-function
                 . bh-syntactic-face-function)
                ;; Get help from font-lock-syntactic-keywords.
                (parse-sexp-lookup-properties . t)
                (font-lock-extra-managed-props . (composition bh-type))))
  ;; Preprocessor definitions can have backslash continuations
  (setq-local font-lock-multiline t)
  ;; Bh's layout rules mean that TABs have to be handled with extra care.
  ;; The safer option is to avoid TABs.  The second best is to make sure
  ;; TABs stops are 8 chars apart, as mandated by the Bh Report.  --Stef
  (setq-local indent-tabs-mode nil)
  (setq-local tab-width 8)
  (setq-local comment-auto-fill-only-comments t)
  ;; Bh is not generally suitable for electric indentation, since
  ;; there is no unambiguously correct indent level for any given line.
  (when (boundp 'electric-indent-inhibit)
    (setq electric-indent-inhibit t))

  ;; dynamic abbrev support: recognize Bh identifiers
  ;; Bh is case-sensitive language
  ...
  (when (eq 25 emacs-major-version)
    (setq-local electric-pair-inhibit-predicate 'bh-mode--inhibit-bracket-inside-comment-or-default))

  (bh-indentation-mode))

(defun bh-fill-paragraph (justify)
  (save-excursion
    ...
    (let* ((syntax-values (syntax-ppss))
      ...
      (cond
       ...
       (t
        ...
        (end-of-line)
        (bh-fill-paragraph justify))))))


(defun bh-adaptive-fill ()
...
;;;###autoload
(defun bh-forward-sexp (&optional arg)
  "Bh specific version of `forward-sexp'.
...
list marker of some kind), and end of the obstacle."
  ...
  (if (< arg 0)
      (while (< arg 0)
        ...
        ;; skipped over at least one Bh expression, and jump forward until
    ...
    (save-match-data
      (while (> arg 0)
        (when (bh-lexeme-looking-at-token)
...
;;;###autoload
(define-derived-mode bh-literate-mode bh-mode "LitBh"
  "As `bh-mode' but for literate scripts."
  (setq bh-literate
        (save-excursion
          (goto-char (point-min))
          (cond
           ...
           (t bh-literate-default))))
  (if (eq bh-literate 'bird)
  ...
  (setq-local mode-line-process '("/" (:eval (symbol-name bh-literate)))))

;;;###autoload
(define-obsolete-function-alias 'literate-bh-mode 'bh-literate-mode "2020-04")


;;;###autoload
(add-to-list 'auto-mode-alist        '("\\.bs\\'" . bh-mode))


(defun bh-mode-before-save-handler ()
  "Function that will be called before buffer's saving."
  (when bh-stylish-on-save
    (ignore-errors (bh-mode-stylish-buffer))))

;; Provide ourselves:
(provide 'bh-mode)
;;; bh-mode.el ends here
