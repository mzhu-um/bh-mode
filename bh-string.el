;;; bh-string.el --- Bh related string utilities -*- lexical-binding: t -*-
...
(defun bh-string-trim (string)
  "Remove whitespace around STRING.

A Whitespace character is defined in the Bh Report as follows
...
(defun bh-string-literal-encode (str &optional no-quotes)
  "Encode STR according Bh escape rules using 7-bit ASCII representation.
...
This is the dual operation to `bh-string-literal-decode'."
                    ...
                    (let ((lc lastc))
                      ...
                      (if (>= c 128) ;; if non-ASCII code point
                        ...
                        (if (or (and (= lc 14) (= c ?H)) ;; "\SO\&H"
                            ...
                            (concat "\\&" (aref bh-string-literal-encode-ascii-array c))
                          (aref bh-string-literal-encode-ascii-array c)
                          ))))))
...
(defconst bh-string-literal-escapes-regexp
  (concat "[\\]\\(?:"
          (regexp-opt (append
...
See Bh Report Sect 2.6,
URL `http://www.bh.org/onlinereport/bh2010/bhch2.html#x7-200002.6',
for more details.")

(defconst bh-string-literal-decode1-table
...
Used by `bh-string-literal-decode1'.")

(defun bh-string-literal-decode1 (l)
  "Decode a single string literal escape sequence.
L must contain exactly one escape sequence.
This is an internal function used by `bh-string-literal-decode'."
  (let ((case-fold-search nil))
    (cond
     ((gethash l bh-string-literal-decode1-table))
...
(defun bh-string-literal-decode (estr &optional no-quotes)
  "Decode a Bh string-literal.
If NO-QUOTES is nil, ESTR must be surrounded by quotes.

This is the dual operation to `bh-string-literal-encode'."
...
(defun bh-string-split-to-lines (str)
  "Split STR to lines and return a list of strings with preceding and
succeding space removed."
  (when (stringp str)
    (cl-mapcar #'bh-string-chomp (split-string str "\n"))))

(defun bh-string-trim-prefix (prefix str)
...
(defun bh-string-trim-suffix (suffix str)
...
(defun bh-string-drop-qualifier (ident)
...
(defun bh-mode-message-line (str)
  "Echo STR in mini-buffer.
Given string is shrinken to single line, multiple lines just
disturbs the programmer."
  (unless (active-minibuffer-window)
    (message "%s" (bh-mode-one-line str (frame-width)))))

(defun bh-mode-one-line (str &optional width)
